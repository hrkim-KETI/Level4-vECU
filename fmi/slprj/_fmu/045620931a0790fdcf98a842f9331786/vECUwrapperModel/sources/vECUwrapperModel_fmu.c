...
# define RT_MDL_TYPE      RT_MODEL_vECUwrapperModel_T
#endif
...
#include "vECUwrapperModel_private.h"
...
typedef struct {
    ExtU_vECUwrapperModel_T ExternalInput;
    ExtY_vECUwrapperModel_T ExternalOutput;
    P_vECUwrapperModel_T Parameter;
    B_vECUwrapperModel_T blockBuffer;
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID,
    ...
    if (!functions->logger) {
        ...
        return NULL;
    } else if (strcmp(fmuGUID, "{f2ff0c2f-52e4-e210-6006-501058ad3ade}")) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Wrong GUID %s. Expected %s.", fmuGUID, "{f2ff0c2f-52e4-e210-6006-501058ad3ade}");
        return NULL;
    }
    ...
    modelData->S = vECUwrapperModel_M;
    if (modelData->S == NULL) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
    ...
    modelData->rList[1] = &(vECUwrapperModel_P.stepSize);
    fmu_syncToBuffer();
    modelData->sList[0] = vECUwrapperModel_U.In1;
    modelData->sList[1] = vECUwrapperModel_Y.Out1;
...
fmi2Status fmi2GetFMUstate(fmi2Component c, fmi2FMUstate* FMUstate) {
    ...
    memcpy(&fmuInternalState->ExternalInput, &vECUwrapperModel_U, sizeof(ExtU_vECUwrapperModel_T));
    memcpy(&fmuInternalState->ExternalOutput, &vECUwrapperModel_Y, sizeof(ExtY_vECUwrapperModel_T));
    memcpy(&fmuInternalState->Parameter, &vECUwrapperModel_P, sizeof(P_vECUwrapperModel_T));
    memcpy(&fmuInternalState->blockBuffer, &vECUwrapperModel_B, sizeof(B_vECUwrapperModel_T));
...
fmi2Status fmi2SetFMUstate(fmi2Component c, fmi2FMUstate FMUstate) {
    ...
    memcpy(&vECUwrapperModel_U, &fmuInternalState->ExternalInput, sizeof(ExtU_vECUwrapperModel_T));
    memcpy(&vECUwrapperModel_Y, &fmuInternalState->ExternalOutput, sizeof(ExtY_vECUwrapperModel_T));
    memcpy(&vECUwrapperModel_P, &fmuInternalState->Parameter, sizeof(P_vECUwrapperModel_T));
    memcpy(&vECUwrapperModel_B, &fmuInternalState->blockBuffer, sizeof(B_vECUwrapperModel_T));
    for(size_t i = 0; i < 2; i++) {
...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    if (modelData->loggingOn) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
            fmi2OK, "info", "CommunicationStepSize=%.16g, LocalSolverStepSize=%.16g", communicationStepSize, (double)0.0001);
    }
    if (communicationStepSize < 0 || fabs(communicationStepSize - steps * (double)0.0001) > 1.0E-6 * (double)0.0001) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
                    fmi2Error, "error", "Stepsize must be non-negative and divisible by %.16g", (double)0.0001);
