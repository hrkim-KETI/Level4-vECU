...
# define RT_MDL_TYPE      RT_MODEL_vECUcontroller_T
#endif
...
#include "vECUcontroller_private.h"
...
typedef struct {
    ExtU_vECUcontroller_T ExternalInput;
    ExtY_vECUcontroller_T ExternalOutput;
    P_vECUcontroller_T Parameter;
    B_vECUcontroller_T blockBuffer;
    real_T rListBuffer[2];
    int32_T iListBuffer[1];
    char* sListBuffer[2];
    fmi2Real time;
} FMUInternalState;
...
void fmu_LogOutput(void) {
    FMUModelData *modelData = (FMUModelData *)_instance;
    strncpy(modelData->sOutput[0], modelData->sList[1], modelData->sSize[1]);
}
void fmu_RestoreOutput(void) {
    FMUModelData *modelData = (FMUModelData *)_instance;
    strncpy(modelData->sList[1], modelData->sOutput[0], modelData->sSize[1]);
}
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID,
    ...
    if (!functions->logger) {
        ...
        return NULL;
    } else if (strcmp(fmuGUID, "{3b3ab182-c0e5-143e-e972-ebfcebe8c573}")) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Wrong GUID %s. Expected %s.", fmuGUID, "{3b3ab182-c0e5-143e-e972-ebfcebe8c573}");
        return NULL;
    }
    ...
    modelData->rList = (real_T**)calloc(2, sizeof(real_T*));
    if (!modelData->rList) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    }
    modelData->iList = (int32_T**)calloc(1, sizeof(int32_T*));
    if (!modelData->iList) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    }
    modelData->sList = (char**)calloc(2, sizeof(char*));
    if (!modelData->sList) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    }
    modelData->sSize = (int32_T*)calloc(2, sizeof(int32_T));
    if (!modelData->sSize) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    ...
    modelData->rList[1] = &(vECUcontroller_P.stepSize);
    modelData->iList[0] = &(vECUcontroller_P.portVal);
    fmu_syncToBuffer();
    modelData->sList[0] = vECUcontroller_U.In1;
    modelData->sList[1] = vECUcontroller_Y.Out1;
...
fmi2Status fmi2GetFMUstate(fmi2Component c, fmi2FMUstate* FMUstate) {
    ...
    memcpy(&fmuInternalState->ExternalInput, &vECUcontroller_U, sizeof(ExtU_vECUcontroller_T));
    memcpy(&fmuInternalState->ExternalOutput, &vECUcontroller_Y, sizeof(ExtY_vECUcontroller_T));
    memcpy(&fmuInternalState->Parameter, &vECUcontroller_P, sizeof(P_vECUcontroller_T));
    memcpy(&fmuInternalState->blockBuffer, &vECUcontroller_B, sizeof(B_vECUcontroller_T));
...
fmi2Status fmi2SetFMUstate(fmi2Component c, fmi2FMUstate FMUstate) {
    ...
    memcpy(&vECUcontroller_U, &fmuInternalState->ExternalInput, sizeof(ExtU_vECUcontroller_T));
    memcpy(&vECUcontroller_Y, &fmuInternalState->ExternalOutput, sizeof(ExtY_vECUcontroller_T));
    memcpy(&vECUcontroller_P, &fmuInternalState->Parameter, sizeof(P_vECUcontroller_T));
    memcpy(&vECUcontroller_B, &fmuInternalState->blockBuffer, sizeof(B_vECUcontroller_T));
    for(size_t i = 0; i < 2; i++) {
...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    steps = (int)(communicationStepSize / (double)1e-05 + 0.5);    
    if (modelData->loggingOn) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
            fmi2OK, "info", "CommunicationStepSize=%.16g, LocalSolverStepSize=%.16g", communicationStepSize, (double)1e-05);
    }
    if (communicationStepSize < 0 || fabs(communicationStepSize - steps * (double)1e-05) > 1.0E-6 * (double)1e-05) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
                    fmi2Error, "error", "Stepsize must be non-negative and divisible by %.16g", (double)1e-05);
        returnStatus = fmi2Error;
    } else if (fabs(modelData->time - currentCommunicationPoint) > 1.0E-6 * (double)1e-05) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
