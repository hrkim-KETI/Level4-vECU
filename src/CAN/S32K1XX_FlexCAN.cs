...
{
    public partial class S32K1XX_FlexCAN : BasicDoubleWordPeripheral, IBytePeripheral, ICAN, IKnownSize
    {
        public S32K1XX_FlexCAN(IMachine machine, uint numberOfMessageBuffers = 32, uint enhancedRxFifoSize = 0) : base(machine)
        {
            if(numberOfMessageBuffers != 24 && numberOfMessageBuffers != 32 && numberOfMessageBuffers != 64 && numberOfMessageBuffers != 96)
            {
        ...
        private void DefineRegisters()
        {
            ...
            Registers.ErrorAndStatus1.Define(this)
                ...
                .WithFlag(1, FieldMode.WriteOneToClear, name: "Error Interrupt Flag (ESR1.ERRINT)")
                // .WithTaggedFlag("Bus Off Interrupt Flag (ESR1.BOFFINT)", 2)
                .WithFlag(2, FieldMode.WriteOneToClear, name: "Bus Off Interrupt Flag (ESR1.BOFFINT)")
            ...
            // RegisterMessageBufferInterruptFlags(interruptFlag1, interruptMask1, 8, 8, 16);            
        ...
        private void RegisterMessageBufferInterruptFlags(DoubleWordRegister flagRegister, DoubleWordRegister maskRegister, int start, int fieldStart = 0, int fieldMax = 32)
        {
            ...
            flagRegister.WithChangeCallback((oldValue, newValue) =>
            {
                this.Log(LogLevel.Info, $"Flag Register changed from 0x{oldValue:X} to 0x{newValue:X}");
                UpdateInterrupts();
            });

            maskRegister.WithChangeCallback((oldValue, newValue) =>
            {
                this.Log(LogLevel.Info, $"Mask Register changed from 0x{oldValue:X} to 0x{newValue:X}");
                UpdateInterrupts();
            });
